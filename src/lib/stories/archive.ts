...
import type { StoryGeometryResult } from './geometry';
...
export interface StoryRunRecord {
	...
	title: string;
	seed: number;
	presetId: StoryPreset;
	metrics: StoryMetrics[];
	snapshots: Array<{ capturedAt: string; atlas: StoryAtlas; geometry: StoryGeometryResult }>;
	checkpoint: StoryCheckpoint | null;
	observations: Array<{ time: string; step: number; title: string; detail: string }>;
	samples?: StoryGeneration[];
	interventions?: Array<{
		step: number;
		modelId: string;
		neuron: number;
...
export interface StoryRunSummary {
	...
	latestMetric: StoryMetrics | null;
...
const DB = 'tissue-story-runs';
const MAGIC = new TextEncoder().encode('TISSUE-STORY-V1\n');
const MAX_BYTES = 512 * 1024 * 1024;
const MAX_HEADER_BYTES = 32 * 1024 * 1024;
const CORPUS_ID =
	'tinystories-ascii-v1:67fb35357ec45562152b012c60d4a2d1aec5a95ad9d9a4fc6d534bc97fb4c6db';
const fail = (message: string): never => {
	throw new Error(`Invalid story run: ${message}`);
};
...
const sameConfig = (actual: unknown, preset: StoryPreset): boolean =>
	!!actual &&
	typeof actual === 'object' &&
	Object.entries(STORY_PRESETS[preset]).every(
		([key, value]) => (actual as Record<string, unknown>)[key] === value
	);

function validateTensors(leaves: StoryTensor[], expectedCount: number, deep: boolean): void {
...
export function validateStoryRun(value: unknown, deep = true): StoryRunRecord {
	check(!!value && typeof value === 'object', 'missing record');
	const r = value as StoryRunRecord;
	check(r.version === 1 && r.kind === 'tissue-story-run', 'unsupported record version');
	...
	const config = STORY_PRESETS[r.presetId];
	...
	if (r.checkpoint) {
		...
		check(
			c.version === 1 &&
				c.architecture === 'stories-transformer-v1' &&
		...
		check(
			c.corpus?.id === CORPUS_ID &&
				c.corpus.dataset === 'TinyStories' &&
				c.corpus.vocabulary?.join('') === STORY_CHARACTERS.join(''),
			'unknown corpus or tokenizer'
		);
		const count = storyParameterCount(config);
		...
		if (deep) validateStoryCheckpoint(c);
		modelId = c.modelId;
	}
	...
	for (const snapshot of r.snapshots) {
		...
		check(
			Array.isArray(a.examples) &&
				a.examples.length === 8 &&
				a.examples.every((x) => typeof x === 'string' && x.length === config.context),
		...
		const probeIdentity = JSON.stringify([a.examples, a.positions]);
		check(
		...
		check(
			a.examples.every((example) => [...example].every((c) => STORY_CHARACTERS.includes(c))),
	...
	if (r.samples) {
		...
		for (const s of r.samples) {
			...
			const encoded = encodeStoryPrompt(s.prompt.original, config.context);
			check(
				s.prompt.text === encoded.text &&
					s.prompt.truncatedCharacters === encoded.truncatedCharacters &&
			...
			check(
				Array.isArray(s.tokenIds) &&
					...
					s.tokenIds.every((id) => integer(id, 95)) &&
					s.completion === s.tokenIds.map((id) => STORY_CHARACTERS[id]).join('') &&
					s.cancelled === s.tokenIds.length < s.requestedTokens,
...
export function summarizeStoryRun(record: StoryRunRecord): StoryRunSummary {
	return {
		id: record.id,
		title: record.title,
...
function openDatabase(): Promise<IDBDatabase> {
	return new Promise((resolve, reject) => {
		...
		request.onerror = () => reject(request.error ?? new Error('Could not open story archive'));
	});
}

export async function saveStoryRun(record: StoryRunRecord): Promise<void> {
	validateStoryRun(record, false);
	const database = await openDatabase();
	try {
		await new Promise<void>((resolve, reject) => {
			const tx = database.transaction(['runs', 'summaries'], 'readwrite');
			tx.objectStore('runs').put(record);
			tx.objectStore('summaries').put(summarizeStoryRun(record));
			tx.oncomplete = () => resolve();
			tx.onerror = tx.onabort = () =>
				reject(tx.error ?? new Error('Could not save story evidence'));
		});
	} finally {
		database.close();
	}
...
async function readStore<T>(store: string, id?: string): Promise<T> {
	...
	try {
		return await new Promise<T>((resolve, reject) => {
			...
			request.onsuccess = () => resolve(request.result);
			request.onerror = () => reject(request.error ?? new Error('Could not read story archive'));
		});
	} finally {
		database.close();
	}
...
export const getStoryArchiveWarnings = (): string[] => [...archiveWarnings];
export async function listStoryRuns(): Promise<StoryRunSummary[]> {
	archiveWarnings = [];
	const summaries = await readStore<StoryRunSummary[]>('summaries');
	return summaries
		.filter((s) => {
			const valid =
				s &&
...
export async function loadStoryRun(id: string): Promise<StoryRunRecord | null> {
	const value = await readStore<unknown>('runs', id);
	return value ? validateStoryRun(value) : null;
}

/** Catalog downloads are pinned to their recorded bytes before becoming local evidence. */
export async function loadStoryReference(reference: StoryReference): Promise<StoryRunRecord> {
	check(
		reference &&
			/^\/experiments\/stories-[a-z0-9-]+\.tissue$/.test(reference.file) &&
	...
	const record = decodeStoryRun(buffer);
	check(
		record.presetId === reference.presetId &&
			record.seed === reference.seed &&
			record.metrics.at(-1)?.step === reference.step &&
			storyParameterCount(STORY_PRESETS[record.presetId]) === reference.parameterCount &&
			storyUnitCount(STORY_PRESETS[record.presetId]) === reference.unitCount,
...
export function encodeStoryRun(record: StoryRunRecord): Blob {
	validateStoryRun(record, false);
...
export function decodeStoryRun(buffer: ArrayBuffer): StoryRunRecord {
	check(
	...
	check(
		...
		'expected a .tissue story archive'
	);
	...
	return validateStoryRun(parsed);
}

export async function importStoryRun(file: File): Promise<StoryRunRecord> {
	check(file.size <= MAX_BYTES, 'archive exceeds 512 MB limit');
	return decodeStoryRun(await file.arrayBuffer());
}
export function exportStoryRun(record: StoryRunRecord): void {
	const url = URL.createObjectURL(encodeStoryRun(record));
	const link = document.createElement('a');
	link.href = url;
	link.download = `tissue-stories-${record.presetId}-seed-${record.seed}-step-${record.checkpoint?.step ?? 'observed'}.tissue`;
	link.click();
