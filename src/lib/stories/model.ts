...
import { storyUnitCount, type StoryConfig, type StoryTensor } from './protocol';
...
export function initializeStoryParameters(c: StoryConfig, seed: number): StoryParams {
	...
	const p: StoryParams = {
		wte: random.normal(key(), [96, c.width]).mul(0.02),
		wpe: random.normal(key(), [c.context, c.width]).mul(0.02),
		lmHead: random.normal(key(), [c.width, 96]).mul(0.001),
		layers: []
	};
...
export function storyForward(
	...
	const batch = tokens.shape[0],
		...
		D = c.width;
	let x = rms(
		np.dot(tokens.reshape([-1, 96]), p.wte).add(np.dot(positions.reshape([-1, S]), p.wpe))
	);
...
export function storyLoss(
	...
	return np.mean(
		np
			.sum(storyForward(p, c, tokens, positions, masks).logprobs.mul(targets.reshape([-1, 96])), -1)
			.neg()
	);
}
export function storyBatch(c: StoryConfig, windows: ArrayLike<number>[]) {
	...
	windows.forEach((window, b) => {
		for (let t = 0; t < c.context; t++) {
			input[b * c.context + t] = window[t] ?? 0;
			targets[b * c.context + t] = window[t + 1] ?? 0;
			positions[b * c.context + t] = t;
		}
	});
	return {
		tokens: nn.oneHot(np.array(input).reshape([windows.length, c.context]), 96),
		positions: nn.oneHot(np.array(positions).reshape([windows.length, c.context]), c.context),
		targets: nn.oneHot(np.array(targets).reshape([windows.length, c.context]), 96)
	};
}
export function storyMask(c: StoryConfig, neuron?: number | null) {
	const data = new Float32Array(storyUnitCount(c)).fill(1);
	if (neuron !== undefined && neuron !== null) {
		if (!Number.isInteger(neuron) || neuron < 0 || neuron >= data.length)
			throw new Error('Story neuron is outside the model');
		data[neuron] = 0;
	}
	return np.array(data).reshape([c.layers, c.hidden]);
}
export async function serializeStoryTree(p: StoryParams): Promise<StoryTensor[]> {
	// Sequential reads keep peak staging memory bounded for large optimizer checkpoints.
	const output: StoryTensor[] = [];
...
export function deserializeStoryTree(config: StoryConfig, leaves: StoryTensor[]): StoryParams {
	let i = 0;
	const leaf = () => {
