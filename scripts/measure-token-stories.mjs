 ...
 * pnpm dev, then node scripts/measure-token-stories.mjs --presets small,medium,large --publish
...
const presets = option('--presets', 'small').split(',');
if (presets.some((value) => !['small', 'medium', 'large'].includes(value)))
	throw new Error('Invalid preset');
const seed = Number(option('--seed', '42'));
const targetSteps = Number(option('--steps', '4096'));
if (!Number.isSafeInteger(targetSteps) || targetSteps < 1 || targetSteps > 100000)
	throw new Error('Invalid update budget');
...
const output = resolve(option('--output', `/tmp/tissue-token-stories-${Date.now()}`));
...
try {
	for (const preset of presets) {
		...
		page.on('console', (message) => {
			if (message.text().startsWith('TOKEN_STORY '))
				console.log(`[${preset}] ${message.text().slice(12)}`);
		});
		...
		const initialized = await page.evaluate(
			async ({ preset, seed, backend, provenance }) => {
				const { TokenStoryEngine } = await import('/src/lib/token-stories/engine.ts');
				const { TokenStoryGeometryEngine } =
					await import('/src/lib/token-stories/geometry-engine.ts');
				const engine = new TokenStoryEngine((event) => {
					if (event.type === 'status') console.log('TOKEN_STORY ' + event.message);
					if (event.type === 'metrics')
						console.log(
							`TOKEN_STORY step ${event.metrics.step}: held-out ${event.metrics.validationLoss.toFixed(4)}, train ${event.metrics.trainLoss?.toFixed(4)}`
						);
				});
				const geometry = new TokenStoryGeometryEngine();
				...
				const record = {
					...
					createdAt: now,
					updatedAt: now,
					title: `TinyStories BPE ${preset} · seed ${seed}`,
					seed,
					presetId: preset,
		...
		const steps = [
			...new Set([0, 256, 1024, targetSteps].filter((step) => step <= targetSteps))
		].sort((a, b) => a - b);
		let lastStep = 0;
		for (const step of steps) {
			...
			const capture = await page.evaluate(async () => {
				const { exportTokenStoryRun } = await import('/src/lib/token-stories/archive.ts');
				...
				state.record.samples.push(
					await state.engine.generate('Once upon a time', {
						maxTokens: 128,
						temperature: 0.8,
						topK: 40,
						seed: 71
				...
				exportTokenStoryRun(state.record);
				return {
					...timing,
			...
			await (await download).saveAs(join(output, `token-stories-${preset}-step-${step}.tissue`));
			console.log(`[${preset}] Captured step ${step}: ${JSON.stringify(capture)}`);
			console.log(await page.evaluate(() => window.__story.record.samples.at(-1).completion));
		}
		const final = await page.evaluate(async () => {
			...
			s.record.observations.push({
				...
				detail: `Unit ${selected} was exactly zero after ablation at every prompt position. Intact/ablated inference and 128 requested subword tokens left weights, Adam state, and training RNG unchanged.`
			});
			...
			return {
				...
				timings: s.timings,
				generation,
				samples: s.record.samples,
				corpusId: s.init.corpus.id,
				tokenizerId: s.init.tokenizer.id,
				probe: {
					unit: selected,
		...
		await page.evaluate(async () => {
			const { exportTokenStoryRun } = await import('/src/lib/token-stories/archive.ts');
			exportTokenStoryRun(window.__story.record);
		});
		const fullPath = join(output, `token-stories-${preset}-complete.tissue`);
		...
		if (publish) {
					...
					const published = {
						...
						observations: [
							...record.observations,
							{
								...
								detail:
									'The complete weights and optimizer checkpoint was retained locally by the research runner. This public reference contains raw activations, geometry, learning curves and samples; it cannot resume training.'
							}
						]
					};
					exportTokenStoryRun(published);
				} else exportTokenStoryRun(record);
			});
			const filename = `token-stories-${preset}-seed-${seed}-step-${lastStep}-${final.id.slice(0, 8)}.tissue`;
			...
			const catalog = JSON.parse(
				await readFile('static/experiments/token-stories-index.json', 'utf8')
			);
			catalog.references = [...catalog.references.filter((r) => r.id !== entry.id), entry];
			await writeFile(
				'static/experiments/token-stories-index.json',
		...
		reports.push(final);
		await writeFile(
			join(output, `token-stories-${preset}-report.json`),
